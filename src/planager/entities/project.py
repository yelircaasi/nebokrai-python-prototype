...
class Project:
    def __init__(self, name: str, id: int, tasks: Union[List[str], str] = [], **kwargs) -> None:
        self.name = name
        self.id = id
        self._tasks = Tasks.from_string_iterable(expand_task_segments(tasks) if isinstance(tasks, str) else tasks)
    ...
    def pretty(self, width: int = 80) -> str:
        ...
        tasks = map(lambda x: tabularize(f"{format_number(x.id)}{x.name} (priority {x.priority})", width), self._tasks)
...
class Projects:
    ...
    def pretty(self, width: int = 80) -> str:
        ...
        empty = tabularize("", width)
        format_number = lambda s: (len(str(s)) == 1) * " " + f" {s} │ "
        names = map(lambda x: tabularize(x, width), map(lambda r: format_number(r.id) + f"{r.name}", self._projects.values()))
        return "\n".join(("", topbeam, empty, top, empty, thinbeam, empty, "")) + "\n".join(names) + '\n' + empty + bottombeam
