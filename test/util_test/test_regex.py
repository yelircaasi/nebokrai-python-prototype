...
class RegexesTest:
    ...
    def test_header_and_body(self) -> None:
        ...
        assert re.search(Regexes.header_and_body, s1).groups() == exp1
    ...
    def test_link_namefirst(self) -> None:
        ...
        exp3 = ("Name with sp@ces and $pecial c#ars _ ", "Link with sp@ces and $pecial c#ars _ ")

        assert re.search(Regexes.link_namefirst, s1).groups() == exp1
        assert re.search(Regexes.link_namefirst, s2).groups() == exp2
        assert re.search(Regexes.link_namefirst, s3).groups() == exp3
