...
class RoutineTest:
    ...
    def test_valid_on(self) -> None:
        assert self.routine1.valid_on(PDate())
        assert self.routine2.valid_on(PDate())
        assert self.routine3.valid_on(PDate())

        assert not self.routine1.valid_on(PDate())
        assert not self.routine2.valid_on(PDate())
        assert not self.routine3.valid_on(PDate())
